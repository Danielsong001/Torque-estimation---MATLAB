...
for  i=1:length(C1)
     ...
     if tf(i)>=1/3*tor0 && tf(i)<=2/3*tor0
         ...
         timenew(k)=time(i);
         thetabian(k)=dthetanew(k)-k1*tor0/3;
         k=k+1;
     end
end
...
%     w10c=(1/N)*Inew(h)^10;
    k4c=tfnew(h)-tor0/3;
    coefficient=[k0c,w1c,w2c,w3c,w4c,w5c,w6c,a1c,a2c,a3c,a4c,b1c,b2c,b3c,b4c,a5c,a6c,b5c,b6c,k4c];
...
for i=1:k-1
    C4new(i,1)=dthetanew(i);
    C4new(i,2)=fsonew(i);
    C4new(i,3)=motornew(i);
    C4new(i,4)=Inew(i);
    C4new(i,5)=tfnew(i);
    C4new(i,6)=timenew(i);
    C4new(i,7)=vlinknew(i);
    C4new(i,8)=0;%alink
    C4new(i,9)=vmotornew(i);
    C4new(i,10)=0;%amotor
    C4new(i,11)=tfcal(i);
    C4new(i,12)=error(i);
    C4new(i,13)=errorrate(i);
end

figure(1);
plot(C4new(1:end,6),C4new(1:end,13),'o');
figure(2);
